...
### Jetson에서 Enter로 카메라 이미지 수집

학습 이미지 자체를 모을 때는 아래 스크립트를 Jetson에서 실행합니다.

CSI 카메라:

```bash
python3 u_shape_robot_slam/capture_camera_on_enter.py \
  --camera csi \
  --display \
  --output-dir u_shape_robot_slam/data/captured_images/session_001
```
...
| 조작 | 동작 |
| --- | --- |
| 터미널에서 Enter | 현재 카메라 프레임 저장 |
| 미리보기 창에서 Enter 또는 Space | 현재 카메라 프레임 저장 |
| 터미널에서 `q` 입력 후 Enter | 종료 |
| 미리보기 창에서 `q` 또는 Esc | 종료 |

저장 결과:

| 파일 | 설명 |
| --- | --- |
| `frame_000001_YYYYMMDD_HHMMSS_mmm.jpg` | Enter를 누른 순간의 이미지 |
| `capture_log.csv` | 저장 파일명, 시각, 이미지 크기, 카메라 frame index 기록 |
